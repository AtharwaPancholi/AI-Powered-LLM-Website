...
import { FaArrowLeftLong, FaE } from 'react-icons/fa6'
    ...
    justify-center p-4'>
      ...
      max-w-2xl p-6'>
        <div className='mb-6'>
            ...
            <p className='text-sm text-gray-500'>Enter the 
                title and add yuor video lectures to enhance your 
                course content. </p>
        </div>

        <input type="text" 
        className='w-full border border-gray-300 rounded-md
        p-3 text-sm focus:outline-none focus:ring-2
        ...
        <div className='flex gap-4 mb-6'>
            ...
            <button className='px-5 py-2 cursor-pointer rounded-md bg-[black]
            text-white hover:bg-gray-600
            transition-all text-sm font-medium shadow' disabled={loading} onClick={handleCreateLecture}>{loading ? <ClipLoader size={30}
            color='white'/> : "+ Create Lecture"}</button>
        ...
        <div className='space-y-2'>
            {lectureData?.map((lecture, index)=> (
                ...
                justify-between items-center p-3 text-sm
                font-medium text-gray-700'>
                    <span>Lecture - {index+1}: {lecture.lectureTitle}</span>
                    <FaEdit className='text-gray-500
                    hover: text-gray-700 cursor-pointer' onClick={()=>navigate(`/editlecture/${courseId}/${lecture._id}`)}/>
                </div>
            ))}
        </div>
      </div>
